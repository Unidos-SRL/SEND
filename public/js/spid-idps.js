...
var idps = [
    ...
    {"organization_name": "TI Trust Technologies srl", "entity_id": "https://login.id.tim.it/affwebservices/public/saml2sso", "logo_uri": "img/spid-idp-timid.svg"},{"organization_name": "TeamSystem s.p.a.", "entity_id": "https://spid.teamsystem.com/idp", "logo_uri": "img/spid-idp-teamsystemid.svg"}]
  ...
  if (spid_elements.length > 0 ) {
    fetch(queryURL)
     ...
     .catch(function (error) {
      ...
      let spid_elements = document.querySelectorAll('ul[data-spid-local]')
      for (var u = 0; u < spid_elements.length; u++) {
          for (var i = 0; i < idps.length; i++) { spid_addIdpEntry(idps[i], spid_elements[u]); }
      }
  }
...
function spid_addIdpEntry(data, element) {
  ...
  if (element.id.indexOf('get')!== -1) {
    li.innerHTML = `<a href="https://segreteriacloud.eu/loginServiziperlapa.php?idp=${data['entity_id']}&redirect_uri=${url_site}/auth/spid&cf_ente=${codice_fiscale}"><span class="spid-sr-only">${data['organization_name']}</span><img src="${data['logo_uri']}" alt="${data['organization_name']}" /></a>`
  };
  element.prepend(li)
}
